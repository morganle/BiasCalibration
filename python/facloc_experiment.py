# -*- coding: utf-8 -*-
"""
Experiment based on the facloc simulation model for bias correction.
"""
import numpy as np
import pandas as pd
...
for m in ms:
    for n in ns:
        ...
        output, hit_boundary = experiment(sim_function, facloc_data, facloc_mles,
                                          true_params, lower_bound, n, m, G, B, seeds, 
                                          ratio=ratio, performance_limits=kpi_limits)
    
        #output[['bootstrap_bias_est','final_bias']].hist()
...
total_output = pd.concat(output) if len(ms)*len(ns)>1 else output
total_output.to_csv('facloc_experiment_redo_secondhalf.csv',header=False,index=False,mode='a')
